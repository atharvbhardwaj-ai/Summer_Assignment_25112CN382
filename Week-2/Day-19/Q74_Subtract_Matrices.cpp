...
void subtractMatrices (int arr1[50][50], int r1, int c1, int arr2[50][50], int r2, int c2){
    int arr[50][50];

    // Added Row, Column Check Clause in Function to not make it dependent on main() function
    if (r1 != r2 || c1 != c2){
        cout << "Subtraction not Possible between given matrices";
    }
    else {
        for (int i = 0; i < r1; i ++){
            for (int j = 0; j < c1; j ++){
        ...
        cout << "The Difference between the 2 Matrices is -" <<endl;
        for (int i = 0; i < r1; i ++){
            for (int j = 0; j <c1; j ++){
                cout << arr[i][j] << " ";
            }
...
int main(){
    ...
    subtractMatrices(M1, r1, c1, M2, r2, c2);
